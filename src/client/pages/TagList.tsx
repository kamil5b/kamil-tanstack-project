...
import { TagResponse, TagsListResponse } from '@/shared/responses/types/tag'
import { Link } from '@tanstack/react-router'
import { Badge } from '../components/ui/badge'

export default function TagList({ data, meta, onPageChange }: TagsListResponse & { onPageChange?: (page: number) => void }) {
  const rows = data ?? []
  const paging = meta ?? { page: 1, totalPages: 1, limit: 10, totalItems: 0 }

  const columns = [
    { key: 'name', header: 'Name' },
    { key: 'color', header: 'Tags', render: (r: TagResponse) => {
    ...
    { key: 'actions', header: 'Actions', render: (r: TagResponse) => <Link to={`/tag/edit/${r.id}`}>Edit</Link> },
  ]
  return (
