...
export default function ProductList({
  ...
  meta,
  onPageChange,
}: ProductsListResponse & { onPageChange?: (page: number) => void }) {
  const rows = data ?? [];
  ...
  const columns = [
    { key: "name", header: "Name" },
    {
      key: "totalPrice",
      header: "Total Price",
      render: (r: ProductResponse) => `$${(r.totalPrice ?? 0).toFixed(2)}`,
    },
    {
      key: "items",
      header: "Items",
      render: (r: ProductResponse) => r.items?.length ?? 0,
    },
    {
      key: "tags",
      header: "Tags",
    ...
    },
    {
      ...
      header: "Actions",
      render: (r: ProductResponse) => (
        <Link to={`/product/edit/${r.id}`}>Edit</Link>
      ),
    },
  ];
  return (
