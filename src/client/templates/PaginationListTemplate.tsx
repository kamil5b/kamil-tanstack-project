...
export default function PaginationListTemplate<T extends Record<string, any>>(props: {
	title?: string
	columns?: Column<T>[]
				...
				<TableBody>
					{data.map((row, idx) => (
						<TableRow key={(row as any).id ?? idx}>
							{merged.map((c) => (
								<TableCell key={c.key}>{c.render ? c.render(row) : (row as any)[c.key]}</TableCell>
							))}
						</TableRow>
					))}
				</TableBody>
					...
					<PaginationContent>
						...
						{Array.from({ length: meta.totalPages }).map((_, i) => (
							<PaginationItem key={i}>
								<PaginationLink isActive={meta.page === i + 1} onClick={() => onPageChange?.(i + 1)}>
									{i + 1}
								</PaginationLink>
							</PaginationItem>
						))}
