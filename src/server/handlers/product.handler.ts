...
export const getProductList = createServerFn({
    method: 'GET',
}).inputValidator(PaginationRequestSchema).handler(async ({ data }) => {
    const svc = initInjection();
    return await svc.productSvc.listProduct({ page: data.page, limit: data.limit });
});
...
export const createProduct = createServerFn({
    method: 'POST',
}).inputValidator(CreateProductRequestSchema).handler(async ({ data }) => {
    const svc = initInjection();
...
export const updateProduct = createServerFn({
    method: 'POST',
}).inputValidator(UpdateProductRequestSchema).handler(async ({ data }) => {
    const svc = initInjection();
