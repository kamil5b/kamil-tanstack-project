import type {
  TagRepository,
  TagService,
} from "@/server/interfaces/tag.interface";
...
import type { TagsListResponse } from "@/shared/responses/types/tag";
...
export class TagServiceImpl implements TagService {
  ...
  async updateTag(req: UpdateTagRequest): Promise<void> {
    if (!req || !("id" in req) || !req.id)
      throw new Error("Tag id required for update");
...
export const createTagService = (repo: TagRepository): TagService =>
  new TagServiceImpl(repo);
