...
export class ProductServiceImpl implements ProductService {
    ...
    async updateProduct(req: UpdateProductRequest): Promise<void> {
        if (!req || !("id" in req) || !req.id) throw new Error("Product id required for update");
...
export const createProductService = (repo: ProductRepository): ProductService => new ProductServiceImpl(repo);